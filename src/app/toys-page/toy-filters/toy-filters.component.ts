...
export class ToyFiltersComponent implements OnInit {
  ...
  setSliders(filters: FilterObjInterface, toyService: ToyServiceService) {
    ...
    noUiSlider.create(yearSlider, {
      start: [
        this.toyService.filters.beginYear as number,
        this.toyService.filters.endYear as number,
      ],
      behaviour: 'drag',
      step: 1,
      connect: true,
    ...
    noUiSlider.create(amountSlider, {
      start: [
        this.toyService.filters.beginAmount as number,
        this.toyService.filters.endAmount as number,
      ],
      behaviour: 'drag',
      step: 1,
      connect: true,
    ...
    yearSlider?.noUiSlider?.on('slide', function (values, handleBar) {
      yearOutput[handleBar].textContent = values[handleBar]
        .toString()
        .slice(0, 4);
    ...
    amountSlider?.noUiSlider?.on('slide', function (values, handleBar) {
      amountOutput[handleBar].textContent = values[handleBar]
        .toString()
        .slice(0, 2);
