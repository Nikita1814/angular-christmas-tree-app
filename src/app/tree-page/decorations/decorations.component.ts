...
export class DecorationsComponent implements OnInit {
  ...
  setDecorations(color: string, anim: string) {
    this.toyService.treeSettings.lightsOn === true &&
    this.toyService.treeSettings.lightsColor === color
      ? (this.toyService.treeSettings.lightsOn = false)
      : (this.toyService.treeSettings.lightsOn = true);
    this.toyService.treeSettings.lightsColor = color;
  ...
  playMusic() {
    ...
    const jingleBells = document.querySelector(
      '.jingle-bells'
    ) as HTMLAudioElement;
    this.toyService.treeSettings.music
      ? jingleBells.pause()
      : jingleBells.play();
  }
  setSnow() {
